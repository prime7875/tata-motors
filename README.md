...
data = pd.read_csv("D:/all_datasets/TM.csv")
...
### to find the index with its name 

row_199 = data.iloc[199]
print(row_199.name)


data_split = '2023-01-31'
...
def plot_train_test_by_q(train_env, test_env, Q, algorithm_name,data_split):
    ...
    layout = {
        ...
        'showlegend': False,
         'shapes': [
             {'x0': data_split, 'x1': data_split, 'y0': 0, 'y1': 1, 'xref': 'x', 'yref': 'paper', 'line': {'color': 'rgb(0,0,0)', 'width': 1}}
         ],
        'annotations': [
...
plot_train_test_by_q(Environment(train),Environment(test),Q,'DQN',data_split)
...
Q, total_losses, total_rewards = train_ddqn(Environment(train))
...
plot_train_test_by_q(Environment(train), Environment(test), Q, 'Double DQN',data_split)
...
Q , total_losses,total_rewards = train_dddqn(Environment(train))
...
plot_train_test_by_q(Environment(train),Environment(test),Q,'Dueling Double DQN',data_split)
